...
def test_depositar(mockContaBancaria, valor, saldo_esperado):
    saldo = mockContaBancaria.depositar(valor)
    assert saldo == saldo_esperado


@pytest.mark.parametrize("valor", [0, -10])
def test_depositar_valor_negativo(mockContaBancaria, valor):
    with pytest.raises(ValorInvalidoError):
        mockContaBancaria.depositar(valor)


def test_sacar_reduz_o_saldo(conta_com_saldo):
...
@pytest.mark.parametrize("valor", [0, -10])
def test_sacar_valor_invalido(conta_com_saldo, valor):
    with pytest.raises(ValorInvalidoError):
        conta_com_saldo.sacar(valor)


def test_sacar_acima_do_saldo_levanta_erro(conta_com_saldo):
...
def test_transferir_move_valor_entre_contas(conta_com_saldo, outra_conta_com_saldo):
...
def test_transferencia_invalida_preserva_saldos(conta_com_saldo, outra_conta_com_saldo):
    with pytest.raises(SaldoInsuficienteError):
...
def test_saldo_inicial_negativo_levanta_erro():
    with pytest.raises(ValorInvalidoError):
